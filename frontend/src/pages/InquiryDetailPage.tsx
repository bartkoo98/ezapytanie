  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        {(inquiry.deliveryLocation || inquiry.termsAndConditions) && (
          <div className="space-y-2 pt-2 border-t border-gray-100">
            {inquiry.deliveryLocation && (
              <div className="text-sm text-gray-600">
                <span className="font-medium text-gray-700">Miejsce realizacji: </span>
                {inquiry.deliveryLocation}
              </div>
            )}
            {inquiry.termsAndConditions && (
              <div className="text-sm text-gray-600">
                <span className="font-medium text-gray-700">Warunki realizacji i płatności: </span>
                {inquiry.termsAndConditions}
              </div>
            )}
          </div>
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-400 flex items-center gap-1.5 mt-0.5">
            {user?.role === 'CONTRACTOR' ? (
              <><EyeOff size={12} /> Oferty konkurencji pozostają trwale utajnione. Po zakończeniu terminu Zamawiający dokona wyboru zwycięskiej oferty.</>
            ) : bidsRevealed ? (
              <><Eye size={12} /> Oferty są widoczne</>
            ) : (
              <><EyeOff size={12} /> Oferty są zapieczętowane i zostaną odpieczętowane automatycznie po zakończeniu terminu.</>
            )}
          </p>
        </div>
        ...
        {loadingOffers ? (
          ...
          <div className="space-y-3">
            {offers.map((offer) => {
              ...
              const isCompetitorOffer = offer.contractorId === null;

              if (isCompetitorOffer) {
                return (
                  <div
                    key={offer.id}
                    className={`rounded-lg border p-4 ${isWinner ? 'border-green-300 bg-green-50' : 'border-gray-200 bg-gray-50'}`}
                  >
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-400 italic">Oferta zapieczętowana (Konkurencja)</p>
                      {isWinner && (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 space-y-1">
                      ...
                      {offer.contractorInstitutionName && (
                        <p className="text-sm font-semibold text-gray-700">{offer.contractorInstitutionName}</p>
                      )}

                      {offer.contractorCompanyDetails && (
                        <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs text-gray-500">
                          {offer.contractorCompanyDetails.nip && (
                            <span>NIP: {offer.contractorCompanyDetails.nip}</span>
                          )}
                          {offer.contractorCompanyDetails.regon && (
                            <span>REGON: {offer.contractorCompanyDetails.regon}</span>
                          )}
                          {offer.contractorCompanyDetails.contactPhone && (
                            <span>Tel: {offer.contractorCompanyDetails.contactPhone}</span>
                          )}
                          {offer.contractorCompanyDetails.contactEmail && (
                            <span>Email: {offer.contractorCompanyDetails.contactEmail}</span>
                          )}
                          {offer.contractorCompanyDetails.address && (
                            <span className="col-span-2">Adres: {offer.contractorCompanyDetails.address}</span>
                          )}
                        </div>
                      )}
                      ...
                      {offer.notes && offer.notes !== 'Zapieczętowane' && (
                        <p className="text-xs text-gray-500">
                          <span className="font-medium">Opis oferty:</span> {offer.notes}
                        </p>
                      )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          {offerSuccess ? (
            ...
            <form onSubmit={handleSubmitOffer} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Opis oferty i warunki wykonania</label>
                <textarea
                  rows={3}
                  value={offerNotes}
                  onChange={(e) => setOfferNotes(e.target.value)}
                  placeholder="Opisz swoją ofertę, warunki realizacji i płatności…"
